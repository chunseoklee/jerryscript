...
class rcs_record_iterator_t
{
 ...
 protected:
  ...
  void access (access_t access_type, T* data)
  {
    ...
    {
      current_offset = _current_pos_p - (uint8_t *) _record_start_p;
    }
    else
    {
      /* skip already iterated nodes */
      left_in_node = node_data_space_size - ((uint8_t *) _record_start_p -
                                             _recordset_p->_chunk_list.get_data_space (node_p));

      current_offset += left_in_node;
      _recordset_p->_chunk_list.get_next (node_p);
      ...
      current_offset = _current_pos_p - current_node_data_space_p;
    }

    left_in_node = node_data_space_size - (_current_pos_p - current_node_data_space_p);
    ...
    else
    {
      ...
      size_t first_chunk_size = node_data_space_size - (_current_pos_p - current_node_data_space_p);
    ...
    if (access_type == ACCESS_SKIP && current_offset + sizeof (T) == record_size)
    {
